...
- **Batch preemption, fully visible.** Batch jobs run in chunks with a checkpointed cursor, and every transition is recorded with its reason: queued (with Jev's deferability rubric), started, preempted (which pressure signal), waiting, aging chunk, resumed, done. The page shows the table per job with a full-history modal. A critical request dipping into the critical reserve raises a yield flag; the job yields at the next chunk boundary and resumes from its cursor when pressure clears. An aging guard prevents starvation.
