...
## Layout
```
netlify/functions/   HTTP + scheduled handlers, thin: parse → core → store → respond
src/core/            pure, no I/O: ladder, breaker, pools, hedge gates, batch step logic, decide()
src/jev/             Decider interface, real Jev client, fake for tests, budget guard + inner breaker
src/store/           Store interface, Blobs implementation with etag CAS, in-memory fallback
public/              index.html, app.js, styles.css
scripts/             e2e.mjs (live-URL checks), load.mjs (optional local load)
...
- After every code change: run `npm test` (unit + property) and `npm run typecheck`. Fix failures before moving on. Do not report a change as done with red tests.
