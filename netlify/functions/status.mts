...
export default async (req: Request, _context: Context) => {
  ...
  const cache = Object.values(state.classCache).sort((a, b) => b.at - a.at).slice(0, 8);
  return json(
    200,
    {
      session: sid,
      ...
      jev: { off: state.jev.off, minuteCount: state.jev.minuteCount, dayCount: state.jev.dayCount, totalCalls: state.jev.totalCalls, consecutiveErrors: state.jev.consecutiveErrors, innerBreakerOpenUntil: state.jev.openUntil, last: state.jev.last, lastError: state.jev.lastError, timeoutMs: JEV_BUDGET.timeoutMs },
      classifications: cache,
      counters: state.counters,
      jobs: jobs.map((j) => ({ id: j.id, description: j.description, items: j.items, cursor: j.cursor, state: j.state, deferability: j.deferability, deferabilityConfidence: j.deferabilityConfidence, deferabilityDecider: j.deferabilityDecider, preemptions: j.preemptions, resumes: j.resumes, resumeAfter: j.resumeAfter, lastEvent: j.history[j.history.length - 1] ?? null })),
      telemetry: state.telemetry.slice(-20),
    },
    { "x-tier": `${state.tier} ${TIER_NAMES[state.tier]}`, "x-breaker": state.breaker.state.toLowerCase().replace("_", "-"), "x-decider": state.decider, "x-store": ctx.degraded ? "degraded" : "ok" },
  );
};
